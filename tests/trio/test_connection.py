...
from ..asyncio.utils import alist
...
class ClientConnectionTests(AssertNoLogsMixin, IsolatedTrioTestCase):
    LOCAL = CLIENT
    REMOTE = SERVER
    ...
    async def asyncSetUp(self):
        ...
        self.connection = Connection(
            self.nursery, stream, protocol, close_timeout=2 * MS
        )
        self.remote_connection = InterceptingConnection(
            self.nursery, remote_stream, remote_protocol
        )
    ...
    async def test_recv_during_recv(self):
        ...
        self.nursery.start_soon(self.connection.recv)
        await trio.testing.wait_all_tasks_blocked()
    ...
    async def test_recv_during_recv_streaming(self):
        ...
        self.nursery.start_soon(alist, self.connection.recv_streaming())
        await trio.testing.wait_all_tasks_blocked()
    ...
    async def test_recv_cancellation_while_receiving(self):
        ...
        async def send_fragments():
            await self.remote_connection.send(fragments())

        self.nursery.start_soon(send_fragments)
        await trio.testing.wait_all_tasks_blocked()
    ...
    async def test_recv_streaming_cancellation_while_receiving(self):
        ...
        async def send_fragments():
            iterator = fragments()
            with self.assertRaises(ConnectionClosedError):
                await self.remote_connection.send(iterator)
            await iterator.aclose()
    ...
    async def test_close_timeout_waiting_for_close_frame(self):
        ...
        exc = self.connection.protocol.close_exc
    ...
    async def test_acknowledge_ping_on_close(self):
        ...
        async with self.drop_frames_rcvd():  # drop automatic response to ping
            pong_received_ack_on_close = await self.connection.ping(
                "this", ack_on_close=True
            )
            pong_received = await self.connection.ping("that")
        await self.connection.close()
        with trio.fail_after(MS):
            await pong_received_ack_on_close.wait()
        with self.assertRaises(trio.TooSlowError):
            with trio.fail_after(MS):
                await pong_received.wait()
    ...
    async def test_close_timeout(self):
        ...
        self.assertEqual(connection.close_timeout, 42)
