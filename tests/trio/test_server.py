...
class ServerTests(EvalShellMixin, LoggingTestCase, IsolatedTrioTestCase):
    ...
    async def test_process_request_returns_response(self):
        ...
        with self.assertNoLogs("websockets", logging.ERROR):
            async with run_server(handler, process_request=process_request) as server:
                with self.assertRaises(InvalidStatus) as raised:
                    async with connect(get_uri(server)):
                        self.fail("did not raise")
                self.assertEqual(
    ...
    async def test_aclose_stops_accepting_connections(self):
    ...
    async def test_aclose_rejects_connecting_connections(self):
    ...
    async def test_aclose_closes_open_connections(self):
    ...
    async def test_aclose_closes_open_connections_with_code_and_reason(self):
    ...
    async def test_aclose_keeps_connections_open(self):
        """Server waits for client to close open connections."""
        async with run_server() as server:
    ...
    async def test_aclose_keeps_handlers_running(self):
    ...
    async def test_listeners(self):
        ...
        async with run_server() as server:
            sock = server.listeners[0].socket
