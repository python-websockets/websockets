...
from ..asyncio.server import basic_auth, broadcast
...
class Server(trio.abc.AsyncResource):
    ...
    Args:
        listeners: Trio listeners.
        handler: Handler for one connection. Receives a Trio stream.
    ...
    def __init__(
        ...
        self.waiters: dict[ServerConnection, trio.Event] = {}
        self.closing = False
    ...
    def connections(self) -> set[ServerConnection]:
        ...
        return {
            connection
            for connection in self.waiters
    ...
    async def aclose(
        ...
        * Wait until all connection handlers have returned.
        ...
        # Wait until all connection handlers have returned.
        while self.waiters:
            await next(iter(self.waiters.values())).wait()
...
async def serve(
    ...
    *,
    ...
    When using :func:`serve` with :func:`nursery.start <trio.Nursery.start>`,
    you get back a :class:`Server` object. Call its :meth:`~Server.close` method
    ...
    Args:
        ...
        listeners: Preexisting TCP listeners. ``listeners`` replaces ``port``,
            ...
            details.
        ssl: Configuration for enabling TLS on the connection. origins:
        Acceptable values of the ``Origin`` header, for defending
        ...
        create_connection: Factory for the :class:`ServerConnection` managing
            ...
            connection handling.
        task_status: For compatibility with :func:`nursery.start
            <trio.Nursery.start>`.
    ...
    async def stream_handler(stream: trio.abc.Stream, server: Server) -> None:
        async with trio.open_nursery() as nursery:
            try:
                ...
                try:
                    server.waiters[connection] = trio.Event()
                    ...
                    server.waiters.pop(connection).set()
