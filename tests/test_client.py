...
class ConnectTests(unittest.TestCase):
    def test_send_connect(self):
        with unittest.mock.patch("websockets.client.generate_key", return_value=KEY):
            client = ClientConnection(parse_uri("wss://example.com/test"))
    ...
    def test_path(self):
        client = ClientConnection(parse_uri("wss://example.com/endpoint?test=1"))
        request = client.connect()
    ...
    def test_port(self):
        for uri, host in [
            ...
            with self.subTest(uri=uri):
                client = ClientConnection(parse_uri(uri))
                request = client.connect()

                self.assertEqual(request.headers["Host"], host)

    def test_user_info(self):
        client = ClientConnection(parse_uri("wss://hello:iloveyou@example.com/"))
        request = client.connect()
...
class AcceptRejectTests(unittest.TestCase):
    def test_receive_accept(self):
        with unittest.mock.patch("websockets.client.generate_key", return_value=KEY):
            client = ClientConnection(parse_uri("ws://example.com/test"))
        client.connect()
        client.receive_data(
            (
    ...
    def test_basic(self):
        client = ClientConnection(parse_uri("wss://example.com/"))
    ...
    def test_no_subprotocol(self):
        client = ClientConnection(
            parse_uri("wss://example.com/"), subprotocols=["chat"]
        )
        response = self.make_accept_response(client)
        client.receive_data(response.serialize())
        [response] = client.events_received()
...
class MiscTests(unittest.TestCase):
    def test_bypass_handshake(self):
        client = ClientConnection(parse_uri("ws://example.com/test"), state=OPEN)
    ...
    def test_custom_logger(self):
        ...
        with self.assertLogs("test", logging.DEBUG) as logs:
            ClientConnection(parse_uri("wss://example.com/test"), logger=logger)
