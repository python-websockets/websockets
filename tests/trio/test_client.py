...
from ..sync.server import run_unix_server
...
class ClientTests(IsolatedTrioTestCase):
    ...
    async def test_tcp_connection_fails(self):
        ...
        with self.assertRaises(OSError):
            async with connect("ws://localhost:12345"):  # invalid port
                self.fail("did not raise")
...
# Tests for unix_connect() use the test server from the threading implementation
# because the trio implementation doesn't support unix_serve because trio itself
# doesn't provide open_unix_listeners() yet:
